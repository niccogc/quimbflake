...
### 1. Using the Quimb Overlay (Recommended)

The cleanest way is to use the overlay provided by the quimb flake. This automatically adds quimb to the Python package set:
...
```nix
{
  ...
  inputs = {
    ...
    quimb-flake.url = "github:user/quimb-flake";  # or path:/path/to/quimb-flake
  };
      ...
      devShells.${system}.default = pkgs.mkShell {
        packages = [
          (pkgs.python313.withPackages (ps: with ps; [
            # quimb is now available like any other package!
            quimb
            autoray
            cotengra
            # Quimb dependencies (must be listed explicitly)
            numpy
            scipy
            numba
            cytoolz
...
**Important:** When using the overlay, you **must explicitly list all dependencies**:
- `quimb`, `autoray`, `cotengra` - Custom packages from this flake
- `numpy`, `scipy`, `numba`, `cytoolz`, `tqdm`, `psutil` - Standard nixpkgs packages

Dependencies do not auto-propagate when using `toPythonModule` with external packages.

### 2. Direct Import (Simple but Verbose)

Add quimb to your Python environment by importing this flake as an input:
...
```nix
{
  ...
  inputs = {
    nixpkgs.url = "github:nixos/nixpkgs/nixos-unstable";
    quimb-flake.url = "path:/path/to/this/flake";
  };
      ...
      devShells.${system}.default = pkgs.mkShell {
        packages = [
          (pkgs.python313.withPackages (ps: [
            # Add quimb from this flake
            quimb-flake.packages.${system}.python313Packages.quimb
            
            # Add other packages from nixpkgs
            ps.matplotlib
            ps.jupyter
          ]))
        ];
...
```nix
{
  ...
  inputs = {
    nixpkgs.url = "github:nixos/nixpkgs/nixos-unstable";
    quimb-flake.url = "github:user/quimb-flake";
  };
...
## Why Use the Overlay Method?

The overlay method (Example 1) is recommended because:
- **Simple**: Just add `overlays = [ quimb-flake.overlays.default ]`
- **Clean syntax**: Use `ps.quimb` instead of `quimb-flake.packages.${system}.python313Packages.quimb`
- **Consistency**: Treat quimb like any other nixpkgs Python package
- **Multi-version**: Automatically works for both Python 3.12 and 3.13
- **Maintainable**: Easier to read and understand for others
- **Composable**: Works seamlessly with other overlays

**Note on Dependencies:** You must explicitly list all quimb dependencies in your `withPackages` call. This is because `toPythonModule` (used internally by the overlay) does not automatically propagate dependencies from external packages.
